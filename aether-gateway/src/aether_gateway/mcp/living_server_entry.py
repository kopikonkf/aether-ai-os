"""Stable process entrypoint for the Aether Living Machine MCP server."""
...
from .living_server import mcp, MCPAuthMiddleware
...
async def health(_: Request) -> JSONResponse:
    return JSONResponse({
        "status": "ok",
        "service": "aether-living-machine-mcp",
        "authentication_configured": bool(os.getenv("AETHER_MCP_TOKEN", "").strip() or os.getenv("AETHER_MCP_OPERATOR_TOKEN", "").strip()),
    })
...
def build_http_app() -> Starlette:
    return Starlette(
        routes=[
            ...
            Mount("/", app=MCPAuthMiddleware(mcp.streamable_http_app())),
        ],
        lifespan=lifespan,
    )
