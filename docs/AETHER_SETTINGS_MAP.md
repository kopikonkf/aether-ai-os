...
| `GH_TOKEN` (makan.txt) | `github_pat_...` | Integrator (push/PR) | Ship-loop ke GitHub | Push gagal |

> ⚠️ **Temuan penting sesi ini:** key Gemini boleh banyak, tapi **rate limit dihitung per PROJECT, bukan per key**. Bikin 10 key di 1 project = tetap 1 kuota. Kalau mau "cadangan kuota", harus beda project.
...
|---|---|---|
| Secret/key | `.env` (tidak di git) + `makan.txt` (arsip Dee) | Tidak bocor ke repo |
