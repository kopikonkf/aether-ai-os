...
- Founder-preferred initial TTS order:
  - Google Cloud TTS;
  - OpenAI exact-text TTS;
  - Cartesia;
  - other conformed hosted provider;
  - local/open-weight TTS;
  - gTTS emergency fallback.
- Source contracts now cover:
  - provider-neutral error taxonomy;
  - explicit 429 rate-limit versus quota/billing exhaustion classification;
  - bounded exponential retry with deterministic jitter and `Retry-After` handling;
...
- Current classification:
  ...
  - persistent AETHER_HOME budget/circuit state: `NOT IMPLEMENTED`;
  - live fallback: `NO`;
  - `ACTIVE`: `NO`;
  - `FOUNDER-PROVEN`: `NO`.
