...
def backup_sqlite(source: Path, destination: Path) -> dict[str, Any]:
    ...
    if destination_check != "ok":
        raise RuntimeError(f"snapshot SQLite quick_check failed for {destination}: {destination_check}")
    return {
...
def import_snapshot(snapshot: Path, destination: Path, *, allow_nonempty: bool = False) -> dict[str, Any]:
...
def main() -> int:
    ...
    if args.command == "export":
        ...
        result = import_snapshot(args.snapshot, args.destination, allow_nonempty=args.allow_nonempty)
